 ...
 */
export function currentSeason(date, hemisphere = 'northern') {
    const m = date.getMonth(); // 0-11
    const north = m === 11 || m <= 1 ? 'winter' :
        m <= 4 ? 'spring' :
            m <= 7 ? 'summer' :
                'autumn';
...
function fieldTriggerSatisfied(trigger, entries) {
...
function evaluateFieldTrigger(rules, entries) {
    if (!rules)
        return true;
...
export function resolveUnlock(chapter, ctx) {
    const rules = chapter.unlock;
    if (!rules || rules.always)
...
export function resolveAll(chapters, ctx) {
    const out = {};
    for (const c of chapters)
        out[c.id] = resolveUnlock(c, ctx);
    return out;
}
